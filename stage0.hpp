...
public:
    int start()
    {
        RenderWindow window(VideoMode(w, h), "Game! [PLAY]");
        window.setFramerateLimit(60);

        bool gameOver=false, isJump = true;
        int a=0,b=-360;
        ...
        while (window.isOpen())
        {
            ...
            else
            {
                ...
                text.setPosition((w-text.getGlobalBounds().width)/6,(h-text.getGlobalBounds().height-40)/2);
